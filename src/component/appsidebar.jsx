...
const AppSideBar = ({ session, profilePicBase64 }) => {
    ...
    return (
        <>
            <div className={clsx("fixed inset-y-0 left-0 z-50 bg-[#0e111b]", "transition-transform duration-300 ease-in-out", "data-[state=close]:-translate-x-full data-[state=open]:translate-x-0", "w-[256px]", "md:relative md:data-[state=close]:translate-x-0 md:transition-all", "md:data-[state=open]:w-[256px] md:data-[state=close]:w-13", "h-screen overflow-hidden group")} data-state="open" id="sidebar">
                <div className="w-full max-w-[256px] px-1 pb-0.5 pt-4 select-none">
                    <div className="flex justify-between items-center group-data-[state=close]:flex-col group-data-[state=close]:gap-2 group-data-[state=close]:items-start">
                        <h1 className="font-medium text-[#e5ebfa] text-[20px] outline-offset-1 pl-2.5">Airi</h1>
                        <div onClick={() => { const sidebar = document.getElementById("sidebar"); const newState = sidebar.getAttribute('data-state') === 'open' ? 'close' : 'open'; sidebar.setAttribute('data-state', newState); }} className="text-[14px] outline-offset-1 flex items-center justify-center hover:bg-[#ffffff14] transition-all duration-100 cursor-pointer bg-transparent text-[#c2cadf] font-medium w-9 h-9 rounded-lg">
                    ...
                    <div className="flex flex-col gap-1 pt-1 items-center group-data-[state=close]:items-start">
                        <button className="group-data-[state=close]:w-9 group-data-[state=close]:h-9 flex w-full gap-2 h-10 items-center px-2 py-1.5 rounded-xl hover:bg-[#ffffff14] cursor-pointer transition-all duration-100 ease-in-out">
                            ...
                            <span className="group-data-[state=close]:hidden flex min-h-6 w-full font-medium items-center gap-1.5 text-start text-[14px] text-[#e5ebfa]">New chat</span>
                        </button>
                        <button className="group-data-[state=close]:w-9 group-data-[state=close]:h-9 flex w-full gap-2 h-10 items-center px-2 py-1.5 rounded-xl hover:bg-[#ffffff14] cursor-pointer transition-all duration-100 ease-in-out">
                            <span>
                            ...
                            <span className="group-data-[state=close]:hidden flex min-h-6 w-full font-medium items-center gap-1.5 text-start text-[14px] text-[#e5ebfa]">Library</span>
                        </button>
                        <button className="group-data-[state=close]:w-9 group-data-[state=close]:h-9 flex w-full gap-2 h-10 items-center px-2 py-1.5 rounded-xl hover:bg-[#ffffff14] cursor-pointer transition-all duration-100 ease-in-out">
                            <span>
                            ...
                            <span className="group-data-[state=close]:hidden flex min-h-6 w-full font-medium items-center gap-1.5 text-start text-[14px] text-[#e5ebfa]">Memory</span>
                        </button>
                    </div>
                    ...
                    <div className="group-data-[state=close]:hidden">
                        {!session && (
                            <p className="px-2.5 pb-6 font-medium text-[#c2cadf] text-[14px]">Sign in to save our conversations.</p>
                        )}
                    ...
                    <div className="absolute bottom-2 w-full max-w-[256px]">
                        ...
                        {!session && (
                            <button onClick={() => window.location.href = "/login"} className="group-data-[state=close]:w-9 group-data-[state=close]:h-9 flex w-full gap-2 h-10 items-center px-1.5 py-1.5 rounded-xl hover:bg-[#ffffff14] cursor-pointer transition-all duration-100 ease-in-out">
                                ...
                                <span className="group-data-[state=close]:hidden flex min-h-6 w-full font-medium items-center gap-1.5 text-start text-[14px] text-[#e5ebfa]">Sign in</span>
                            </button>
                        )}
                        {/* if signed */}
                        {session && (
                            <div className="relative">
                                ...
                                >
                                    ...
                                    <span className="group-data-[state=close]:hidden flex min-h-6 w-full font-medium items-center gap-1.5 text-start text-[15px] truncate text-[#e5ebfa]">
                                        {session.user.name}
                                    </span>
                                </button>
                                <div
                                ...
                                >
                                    <button onClick={() => setSettingsOpen(true)} className="font-medium cursor-pointer flex w-full items-center gap-2 rounded-lg px-3 py-2 text-[14px] hover:bg-[#ffffff14] transition-all duration-100">
                                        Settings
                                    </button>
                                    <button onClick={() => window.location.href = "/auth/logout"} className="font-medium cursor-pointer flex w-full items-center gap-2 rounded-lg px-3 py-2 text-[14px] hover:bg-[#ffffff14] transition-all duration-100">
                                        Sign out
                                    </button>
                                </div>
                            </div>
            ...
            {settingsOpen && (
                <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm flex items-center justify-center" onClick={(e) => e.target === e.currentTarget && setSettingsOpen(false)}>
